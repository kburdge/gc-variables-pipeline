...
**WCS note:** place the astrometric solutions (`{target}_{seg}_{det}_wcs_lw.fits`
for SW, `_wcs_gaia.fits` for LW -- from the Zenodo data products, or generated by
`scripts/06_astrometry_lw_gaia.py`) in `paths.astrometry_dir` **before** building
cubes: the cube builder injects the calibrated Gaia-tied WCS into the cube
headers at creation. Without them the cubes keep the raw pointing WCS
(~0.05-0.2" off Gaia) and a warning is printed; coordinates in downstream
catalogs are computed through the astrometry solutions either way.
