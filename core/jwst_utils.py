...
def get_rate_files(target, segment=None, detector=None, data_root=DATA_ROOT):
    """
    Get list of rate files for a given target/segment/detector.
    """
    if target not in TARGETS:
        raise ValueError(f"Unknown target: {target}")
    ...
    rate_files = []
    for seg in segments:
        # Rate files can be in detector1_output or main segment dir
        for subdir in [tgt_info['ramp_subdir'], '']:
            rate_dir = os.path.join(data_root, target, seg, subdir) if subdir else os.path.join(data_root, target, seg)
            if not os.path.isdir(rate_dir):
                continue
            for det in detectors:
                pattern = os.path.join(rate_dir, f'*_{det}_rate.fits')
                rate_files.extend(glob.glob(pattern))
    
    return sorted(list(set(rate_files)))


def list_available_data(data_root=DATA_ROOT):
    """Print summary of available data."""
    print(f"Data root: {data_root}\n")
    for target, info in TARGETS.items():
        print(f"Target: {target}")
        for seg in info['segments']:
            ramp_dir = os.path.join(data_root, target, seg, info['ramp_subdir'])
            if os.path.isdir(ramp_dir):
                ramps = glob.glob(os.path.join(ramp_dir, '*_ramp.fits'))
                print(f"  {seg}: {len(ramps)} ramp files")
            else:
...
def consolidate_nearby_detections(sources, max_dist=1.5):
    """
    Consolidate multiple detections of the same source.

    For sources within max_dist pixels, merge to single detection using
    flux-weighted centroid. This eliminates duplicate detections at the
    same position (0.0px separation).

    Args:
        sources: astropy Table from DAOStarFinder
        max_dist: maximum distance in pixels to consider as duplicates

    Returns:
        Consolidated astropy Table with merged sources
    """
    ...
    for i in range(n):
        ...
        # Find all sources within max_dist of source i
        nearby_mask = dist_matrix[i] <= max_dist
        group_indices = np.where(nearby_mask)[0]

        if len(group_indices) == 1:
            # Isolated source, keep as is
            consolidated.append(sources[i])
            used[i] = True
        else:
            # Multiple detections - compute flux-weighted centroid
            group = sources[group_indices]
            fluxes = group['flux']
            total_flux = np.sum(fluxes)

            # Flux-weighted position
            x_weighted = np.sum(group['xcentroid'] * fluxes) / total_flux
            y_weighted = np.sum(group['ycentroid'] * fluxes) / total_flux

            # Keep other properties from brightest source
            brightest_idx = np.argmax(fluxes)
            brightest = group[brightest_idx]

            # Create merged source - create a new row by modifying a copy of brightest
            # Convert to dict, modify, then append
            merged_dict = {}
            for col in group.colnames:
                merged_dict[col] = brightest[col]
            merged_dict['xcentroid'] = x_weighted
            merged_dict['ycentroid'] = y_weighted
            merged_dict['flux'] = total_flux
            merged_dict['peak'] = np.max(group['peak'])

            consolidated.append(merged_dict)
            used[group_indices] = True

    # Create new table from consolidated sources
    if len(consolidated) == 0:
        return Table(names=sources.colnames, dtype=sources.dtype)

    # Convert list of dicts to Table
    result_table = Table(rows=consolidated, names=list(consolidated[0].keys()))
    return result_table
...
def do_aperture_photometry(cube, positions, ap_radius=1.5, high_flux_thresh=50000):
    """
    Perform aperture photometry on a data cube.
    
    Args:
        cube: 3D array (n_frames, ny, nx)
        positions: Nx2 array of (x, y) pixel positions
        ap_radius: aperture radius in pixels
        high_flux_thresh: mask pixels above this value
    
    Returns: flux array (n_frames, n_sources)
    """
    n_frames = cube.shape[0]
    n_src = len(positions)
    aperture = CircularAperture(positions, r=ap_radius)
    hi_mask = cube[0] > high_flux_thresh
    flux = np.zeros((n_frames, n_src), dtype=float)
    
    for i in range(n_frames):
        frame = np.nan_to_num(cube[i], nan=0.0)
...
def test_saturation(flux, sat_thresh=10.0, group_size=9):
    """
    Test if a source is saturating by comparing first vs last frames in groups.
    
    Returns: is_saturated (bool)
    """
...
def get_target_from_header(fits_path):
    """Extract target name from FITS header."""
    with fits.open(fits_path) as hdul:
        return hdul[0].header.get('TARGNAME', 'UNKNOWN')
